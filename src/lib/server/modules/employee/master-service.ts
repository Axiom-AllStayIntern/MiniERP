...
import { estimateSingaporeResidentTax } from '$lib/server/modules/finance/compat';
	...
	async getEmployeeDetailPage(employeeId: string, taxYearParam: string | null) {
		...
		return {
			...
			individualTax: {
				...
				byIncomeType,
				note:
					'Figures sum project-linked payout lines (status confirmed or paid) in the selected calendar year. IRAS reliefs beyond employee CPF are not stored; estimated tax uses resident progressive bands for illustration only 鈥?verify with IRAS / your tax advisor.'
			}
		};
	}
