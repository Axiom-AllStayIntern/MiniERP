/**
 * OCR via Cloudflare Workers AI vision models (e.g. Llama 3.2 Vision).
 * Used when PDF has no text layer or when the upload is a raster image.
 */
...
/** Max raw image bytes sent to Workers AI (before base64). */
...
function extractVisionText(raw: unknown): string {
	...
	if (Array.isArray(content)) {
		const parts = content
			.map((p) => {
				if (typeof p === 'object' && p && 'text' in p) return String((p as { text?: string }).text ?? '');
				return '';
			})
			.filter(Boolean);
		return parts.join('\n');
...
/**
 * Run Workers AI vision OCR on a single image (PNG, JPEG, WebP, GIF as supported by the model).
 */
export async function runWorkersVisionOcr(
	...
	if (input.imageBytes.length > MAX_IMAGE_BYTES) {
		return {
			ok: false,
			error: `Image too large for OCR (${input.imageBytes.length} bytes). Max ${MAX_IMAGE_BYTES} bytes.`
		};
	}

	const model =
		readEnv(env, 'WORKERS_AI_VISION_MODEL').trim() || '@cf/meta/llama-3.2-11b-vision-instruct';
	const mime = normalizeMime(input.mimeType);
	const dataUri = `data:${mime};base64,${uint8ToBase64(input.imageBytes)}`;

	// Cloudflare: image must be inside messages (image_url), not top-level `image` (deprecated).
	...
	const runVision = async () => {
		return env.AI!.run(model as Parameters<NonNullable<Env['AI']>['run']>[0], {
			messages,
			max_tokens: 2048,
			temperature: 0.2
	...
	try {
		...
		try {
			...
			// Llama 3.2 Vision requires a one-time Meta license acknowledgement (`prompt: "agree"`).
			try {
				await env.AI!.run(model as Parameters<NonNullable<Env['AI']>['run']>[0], {
					prompt: 'agree'
				} as Parameters<NonNullable<Env['AI']>['run']>[1]);
			} catch {
				/* already agreed or unrelated error */
			}
			raw = await runVision();
		}
