<script lang="ts">
	...
	const storageKey = $derived(data.invoice.pdfUrl ?? '');

	const displayTitle = $derived.by(() => {
		if (!storageKey) return data.invoice.invoiceNo;
		const tail = storageKey.split('/').pop() ?? storageKey;
		try {
			return decodeURIComponent(tail) || data.invoice.invoiceNo;
		} catch {
			return tail || data.invoice.invoiceNo;
		}
	});
...
<div class="space-y-5">
	...
	<section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
		<div class="border-b border-slate-200 px-5 py-4">
			<p class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Customer invoice</p>
		...
		<div class="grid gap-px bg-slate-200 sm:grid-cols-2">
			<div class="bg-white px-5 py-3">
				<p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Invoice no.</p>
				<p class="mt-1 text-sm text-slate-900">{data.invoice.invoiceNo}</p>
			</div>
			<div class="bg-white px-5 py-3">
				<p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Customer</p>
				<p class="mt-1 text-sm text-slate-900">{data.customerName}</p>
			</div>
			<div class="bg-white px-5 py-3">
				<p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Invoice date</p>
				<p class="mt-1 text-sm text-slate-900">{data.invoice.date}</p>
			</div>
			<div class="bg-white px-5 py-3">
			...
			<div class="bg-white px-5 py-3">
				<p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">GST type</p>
				<p class="mt-1 text-sm text-slate-900">{data.invoice.gstType}</p>
			</div>
			<div class="bg-white px-5 py-3">
				<p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Subtotal</p>
				<p class="mt-1 text-sm text-slate-900">{money(data.invoice.subtotal)}</p>
			</div>
			<div class="bg-white px-5 py-3">
			...
			<div class="bg-white px-5 py-3 sm:col-span-2">
				...
				<p class="mt-1 text-sm text-slate-900">{money(data.invoice.total)}</p>
			</div>
			<div class="bg-white px-5 py-3 sm:col-span-2">
				<p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">Storage key (PDF)</p>
	...
	{#if data.lineItemsPretty}
		<section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
			<div class="border-b border-slate-200 px-5 py-3">
				<h3 class="text-[13px] font-medium text-slate-900">Line items</h3>
			</div>
			<div class="px-5 py-4">
				<pre class="max-h-64 overflow-auto whitespace-pre-wrap break-words font-mono text-xs text-slate-700">{data.lineItemsPretty}</pre>
			</div>
		</section>
	{/if}
