...
export class SettlementService {
	...
	async settleCompanyAllocation(params: {
		...
		for (const ecc of companyComponents) {
			...
			const note = `${ecc.label} √ó ${weightPct}% (company ‚Ü?project)`;
...
export class ProjectStaffingService {
			...
			summaryStats: {
				...
				pendingSettlementLabel:
					draftPayoutsThisMonth.length > 0 || pendingComponentsCount > 0
						? `${draftPayoutsThisMonth.length} draft payout(s) Ë∑?${pendingComponentsCount} unsettled component(s)`
						: 'None',
				staffPctOfTotalCost
			}
		};
	...
	async settleAllForMonth(projectId: string, monthYm: string) {
		...
		return {
			ok: true,
			message: `${monthYm}: posted ${allocLines} company-allocation payout line(s) (${allocSkipped} roster member(s) skippedÈà•Êîèo allocation or nothing to settle), ${compLines} project component line(s).`
		};
	}
