...
- **AI Agent（v1）**：全局聊天入口（右下角）、自然语言意图识别、按角色过滤可执行 action、页面跳转与参数预填（项目页自动注入 `project_id` / `project_name` 上下文）
...
- `POST /api/agent/intent`：接收自然语言与页面上下文，返回 `reply`、`matched_action_id`、`prefill`、`missing_context` 与可选 `action`
